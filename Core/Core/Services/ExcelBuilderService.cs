...
namespace Core.Services
{
    public class ExcelBuilderService : IExcelBuilderService, IDisposable
    {
        ...
        public void CreateExcelDocument(string excelFilePath = null)
        {
            ...
            {
                _guard.Assert(File.Exists(excelFilePath), ExceptionCodeEnum.BadRequest, "excelFilePath نامعتبر است.");
                                ...
                                string excelFilePath = null, bool rightToLeft = true, int maxNoOfRecords = 50000, bool autoFitColumns = true)
        {
            ...
            _guard.Assert(_excelPackage is not null, ExceptionCodeEnum.BadRequest, $"ابتدا باید متد {nameof(CreateExcelDocument)} فراخوانی شود.");
            var dataModelType = typeof(T);
            var propertyNames = _reflectionService.GetMemberNames(dataModelType);
            ...
            _guard.Assert(dataCollection.Count() < 1048576, ExceptionCodeEnum.InternalServerError, "تعداد رکوردها نمی تواند بیش از 1.048.576 باشد.");
            _guard.Assert(propertyNames.Count < 16384, ExceptionCodeEnum.InternalServerError, "تعداد ستون ها نمی تواند بیش از 16.384 باشد.");
