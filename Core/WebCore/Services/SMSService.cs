...
{
    public class SMSService : MessageDispacherService, ISMSService, IDisposable
    {
        private bool disposedValue;

        public async Task Send(SMSRq request)
        {
            var items = new List<SMSRq>
            {
                request
            };
        ...
        public async Task Send(IEnumerable<SMSRq> request)
        {
            ...
            foreach (var message in request)
            {
                ...
                var msg = new SendMessageRq()
                {
                    ...
                    Type = Core.Enums.Messaging.MessageDispatcher.MessageType.SMS,
                    Title = "..",
